...
export function gpBuscarCliente(query) {
  ...
  if (matches.length === 0) {
    gpResultados.innerHTML = '<div class="px-4 py-3 text-xs text-zinc-400 dark:text-zinc-500 italic">Sin resultados</div>';
  } else {
    gpResultados.innerHTML = matches.map((c) => `
      <button data-action="gp-seleccionar" data-cliente-id="${esc(c.id)}" class="w-full text-left px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors border-b border-zinc-100 dark:border-zinc-800 last:border-0">
        <div class="flex items-center justify-between gap-3">
          <div class="min-w-0">
            <div class="text-xs font-medium text-zinc-900 dark:text-zinc-100 truncate">${esc(c.nombreComercial || c.nombreCliente || 'Sin nombre')}</div>
            <div class="text-[11px] text-zinc-400 dark:text-zinc-500 font-mono truncate">${esc(c.nombreCliente || '')}</div>
          </div>
          <span class="text-[10px] font-mono px-2 py-0.5 rounded-full shrink-0 ${
            c.estadoCliente === 'Retirado'
...
function refrescarUI() {
  ...
  elEC.textContent = ec;
  elEC.className = 'text-xs font-medium mt-0.5 ' +
    (ec === 'Activo' ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500 dark:text-zinc-400');

  const elES = document.getElementById('gp-estado-servicio');
  elES.textContent = es;
  elES.className = 'text-xs font-medium mt-0.5 ' +
    (es === 'Activo' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400');
  ...
  if (ec === 'Retirado') {
    btnS.disabled = true;
    btnS.className = 'px-3 py-1.5 text-xs font-medium rounded-md border border-zinc-200 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500 cursor-not-allowed';
  } else {
    btnS.disabled = false;
  }
}
